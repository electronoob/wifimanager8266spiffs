...
void WiFiManager::handleRoot() {
  ...
  String page = WiFiManager::getSpiffsTPL("HTTP_HEAD");
  page.replace("{v}", "Options");
  page += WiFiManager::getSpiffsTPL("HTTP_SCRIPT");
  page += WiFiManager::getSpiffsTPL("HTTP_STYLE");
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  String page = getSpiffsTPL("HTTP_HEAD");
  page.replace("{v}", "Config ESP");
  page += getSpiffsTPL("HTTP_SCRIPT");
  page += getSpiffsTPL("HTTP_STYLE");
  page += _customHeadElement;
  page += getSpiffsTPL("HTTP_HEAD_END");
    ...
    if (n == 0) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (_minimumQuality == -1 || _minimumQuality < quality) {
          String item = getSpiffsTPL("HTTP_ITEM");
          String rssiQ;
          rssiQ += quality;
  ...
  page += getSpiffsTPL("HTTP_FORM_START");
  char parLength[5];
  ...
  for (int i = 0; i < _paramsCount; i++) {
    ...
    String pitem = getSpiffsTPL("HTTP_FORM_PARAM");
  ...
  if (_sta_static_ip) {
    ...
    String item = getSpiffsTPL("HTTP_FORM_PARAM");
    item.replace("{i}", "ip");
    item.replace("{n}", "ip");
    ...
    item = getSpiffsTPL("HTTP_FORM_PARAM");
    item.replace("{i}", "gw");
    item.replace("{n}", "gw");
  ...
  page += getSpiffsTPL("HTTP_FORM_END");
  page += getSpiffsTPL("HTTP_SCAN_LINK");
...
String WiFiManager::getSpiffsTPL(String filename){
  String path = "/wmt/";
  path += filename;
  path += ".txt";
...
void WiFiManager::handleNotFound() {
  ...
  if (!f) {
    ...
    server->send ( 404, WiFiManager::getContentType(server->uri()), message );                                
  }
  else
  {
      Serial.println("Reading Data from File");
